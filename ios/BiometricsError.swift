...
public enum BiometricsError: Error {
    case userCancel
    case userFallback
    case systemCancel
    ...
    public static func fromLAError(_ error: LAError) -> BiometricsError {
        switch error.code {
        case .userCancel:
            return .userCancel
        case .userFallback:
    ...
    public static func fromOSStatus(_ status: OSStatus) -> BiometricsError {
        switch status {
...
public func handleError(_ error: BiometricsError, promise: Promise) {
    print("Biometric Error: \(error.errorInfo.code) - \(error.errorInfo.message)")
    promise.reject(error)
}

public func handleErrorWithResult(_ error: BiometricsError, promise: Promise) {
    print("Biometric Error (result): \(error.errorInfo.code) - \(error.errorInfo.message)")
    promise.resolve([
        "success": false,
