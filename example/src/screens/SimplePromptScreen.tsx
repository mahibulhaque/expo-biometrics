...
export default function SimplePromptScreen() {
  ...
  const loadCurrentKeyAlias = async () => {
    try {
      const defaultKeyAlias = await ExpoBiometricsModule.getDefaultKeyAlias();
      console.log({ defaultKeyAlias });
      setCurrentKeyAlias(defaultKeyAlias);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const loadAllKeys = async () => {
    try {
      const allKeys = await ExpoBiometricsModule.getAllKeys(null);
      console.log(allKeys);
    } catch (error) {
      console.error(error);
    }
